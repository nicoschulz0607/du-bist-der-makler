...
export const metadata = { title: 'Makler-Anfragen — Admin' }
...
export default async function AdminAnfragenPage() {
  ...
  const profileMap: Record<string, { vorname: string | null }> = {}
  ...
  if (userIds.length > 0) {
    ...
    for (const p of profiles ?? []) {
      profileMap[p.id as string] = { vorname: p.vorname as string | null }
    }
  }
          ...
          <table className="w-full text-left">
            ...
            <tbody>
              {anfragen.map((a) => {
                ...
                const vorname = profileMap[a.user_id as string]?.vorname ?? '—'
                return (
                  <tr key={a.id as string} className="border-b border-[#EEEEEE] last:border-0 hover:bg-[#F9F9F9] transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <p className="text-[14px] font-medium text-text-primary">{vorname}</p>
